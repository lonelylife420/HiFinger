...
def work(target_url,finger_regular):
    try:
...
#多线程处理
@timed_function
def threading_pool(finger_regular,file_path,pool_num=min(300, os.cpu_count() * 4)):
    '''
    多线程处理
    '''
    ...
    with ThreadPoolExecutor(max_workers=pool_num) as executor:
        ...
        for url in urls:
            try:
                ...
                url = url.strip()
                future = executor.submit(work, url, finger_regular)
...
@timed_function
def one_work(target_url,finger_regular):
    target_url = args.url
    ...
    return work(target_url,finger_regular)
...
if __name__ == '__main__':
    try:
        ...
        if len(sys.argv) == 1:
            ...
            result_list.append(one_work(args.url, finger_regular))
            if result_list != []:
                write_to_csv(result_list,args.output)
        elif args.file is not None:
            result_list = threading_pool(finger_regular,args.file,args.pool)
            if result_list != []:
                write_to_csv(result_list,args.output)        
